...
import { X402ApiError } from "../models/errors/x402-api-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | X402ApiError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | X402ApiError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new X402ApiError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new X402ApiError("API error occurred", {
          request,
          response,
          body,
        }),
